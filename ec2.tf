...
The Amazon Web Services (AWS) provider is used to interact with the many resources supported
by AWS. The provider needs to be configured with the proper credentials before it can be used.
*/
provider "aws" {
    region = "ap-south-1"
...
Generates a secure private key and encodes it as PEM. This resource is primarily
intended for easily bootstrapping throwaway development environments.
...
This key pair public key will be registered with AWS to allow logging-in to EC2 instances.
...
// print the public IP of the instance created above
